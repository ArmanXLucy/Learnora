...
This module replaces the SQLite-based DAL so Learnora can run correctly on
Vercel/serverless environments. Persistent application data is stored in
Cloud Firestore.
...
The public Firebase web configuration variables are not used here.
"""
...
def _get_firestore():
    """Return a Firestore client, initializing Firebase Admin once."""
    global _firestore
    ...
    try:
        ...
        if service_json:
            ...
            # Vercel/environment variables sometimes contain literal escaped
            # newlines. json.loads normally decodes them, but this also handles
            # a manually supplied private_key safely.
            ...
            raise RuntimeError(
                "Firebase Admin is not configured. Set "
                "FIREBASE_SERVICE_ACCOUNT_JSON in Vercel."
            )

        app = firebase_admin.initialize_app(cred)

    _firestore = firestore.client(app)
    return _firestore
